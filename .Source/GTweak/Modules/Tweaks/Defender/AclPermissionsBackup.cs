    ...
    internal class AclPermissionsBackup : TaskSchedulerManager
    {
        ...
        internal static void ExportRights()
        {
            try
            {
                ...
                foreach (var entry in _storageRegPaths)
                {
                    ...
                    foreach (var view in new[] { RegistryView.Registry64, RegistryView.Registry32 })
                    {
                        ...
                        try
                        {
                            ...
                            Dictionary<string, RegistryValueBackup> values = new Dictionary<string, RegistryValueBackup>(StringComparer.Ordinal);
                ...
                File.WriteAllText(PathTargets.Files.BackupJsonWD, JsonConvert.SerializeObject(allValues, Formatting.Indented));
                File.WriteAllText(PathTargets.Files.BackupAclWD, JsonConvert.SerializeObject(aclDataDict, Formatting.Indented));
            }
            catch (Exception ex) { ErrorLogger.LogDebug(ex); }
        }
        ...
        {
            if (File.Exists(PathTargets.Files.BackupJsonWD) && File.Exists(PathTargets.Files.BackupAclWD))
            {
                try
                {
                    Dictionary<string, Dictionary<string, RegistryValueBackup>> allValues = JsonConvert.DeserializeObject<Dictionary<string, Dictionary<string, RegistryValueBackup>>>(File.ReadAllText(PathTargets.Files.BackupJsonWD));
                    Dictionary<string, string> aclDataDict = JsonConvert.DeserializeObject<Dictionary<string, string>>(File.ReadAllText(PathTargets.Files.BackupAclWD));
