    ...
    internal class FirewallManager : TaskSchedulerManager
    {
        ...
        protected static void BlockWindowsUpdate(bool state)
        {
            try
            {
                ...
                {
                    (PathTargets.Targets.WindowsUpdate.WorkerCore.Normal, _nameRules["Update"]),
                    (PathTargets.Targets.WindowsUpdate.UsoClient.Normal, $"{_nameRules["Update"]} (Update Orchestrator)")
                },
                executableFile =>
                {
        ...
        {
            try { Parallel.Invoke(() => { ChangeRules(state, PathTargets.Targets.Defender.MpCmdRun.Normal, _nameRules["WDefender"], NET_FW_RULE_DIRECTION_.NET_FW_RULE_DIR_OUT, "blocking Windows Defender database updates"); }); }
            catch (Exception ex) { ErrorLogger.LogDebug(ex); }
        }
    }
}
