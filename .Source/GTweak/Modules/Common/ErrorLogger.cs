...
namespace GTweak.Modules.Common
{
    internal static class ErrorLogger
    {
        ...
        private static async Task SaveDumpToFile(Exception ex, bool isFatal, string memberName)
        {
            try
            {
                using (FileStream stream = new FileStream(PathTargets.Files.ErrorLog, FileMode.Create, FileAccess.Write, FileShare.Read))
                ...
                if (File.Exists(PathTargets.Files.ErrorLog))
                {
                    try
                    {
                        await Task.Delay(50);
                        Process.Start(new ProcessStartInfo { FileName = PathTargets.Files.ErrorLog, UseShellExecute = true });
                    }
