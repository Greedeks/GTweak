    ...
    internal class NetworkProvider : MonitoringProvider
    {
        ...
        internal async Task GetUserIpAddress()
        {
            if (await IsNetworkAvailable())
            {
                ...
                foreach (string url in PathTargets.Links.IpServices)
                {
                    try
                    {
        ...
        {
            if (GlobalOptions.IsUpdateCheckRequired && await IsNetworkAvailable())
            {
                foreach (string api in PathTargets.Links.ReleaseApi)
                {
                    try
                    {
                        ...
                        if (response.IsSuccessStatusCode)
                        {
                            ...
                            if (api.Contains("github"))
                            {
                                ...
                                foundUrl = PathTargets.Links.LatestUpdate.GitHub;
                            }
                            else
                            {
                                ...
                                if (latest != null)
                                {
                                    ...
                                    if (match.Success)
                                    {
                                        foundUrl = $"{PathTargets.Links.LatestUpdate.GitLabBase}{match.Groups["hash"].Value}";
                                    }
                                }
                            }
                            ...
                            {
                                PathTargets.Links.LatestUpdate.Resolved = foundUrl;

                                if (new Version(foundVersion) > new Version(GlobalOptions.CurrentRelease.Short))
                                {
                                    IsNeedUpdate = true;
