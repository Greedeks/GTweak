...
namespace GTweak.Modules.Configuration
{
    internal sealed class HardwareProvider : NetworkProvider
    {
        ...
        internal ImageSource GetProfileImage()
        {
            try
            {
                string avatarPath = RegistryHelper.GetValue($@"HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\AccountPicture\Users\{WindowsIdentity.GetCurrent().User?.Value}", "Image1080", string.Empty);
        ...
        private void GetWallpaperImage()
        {
            try
            {
                string wallpaperPath = RegistryHelper.GetValue(@"HKEY_CURRENT_USER\Control Panel\Desktop", "WallPaper", string.Empty);

                string filePath = !string.IsNullOrWhiteSpace(wallpaperPath) && File.Exists(wallpaperPath) ? wallpaperPath :
                    Directory.GetFiles(PathTargets.Folders.WallpaperCache, "TranscodedWallpaper*").Select(f => new FileInfo(f)).Where(f => f.Exists).OrderByDescending(f => f.LastWriteTime).FirstOrDefault()?.FullName ?? string.Empty;
        ...
        internal void GetOperatingSystemInfo()
        {
            ...
            int revisionNumber = RegistryHelper.GetValue(regPath, "UBR", 0);
            string release = RegistryHelper.GetValue(regPath, "DisplayVersion", string.Empty);
        ...
        private void GetMotherboardInfo()
        {
            ...
            foreach (string rootPath in new string[] { @"SYSTEM\CurrentControlSet\Enum\PCI", @"SYSTEM\CurrentControlSet\Enum\ACPI" })
            {
                ...
                List<string> devices = RegistryHelper.GetSubKeyNames<List<string>>(Registry.LocalMachine, rootPath);
                if (devices == null)
                {
                    continue;
                }
                ...
                foreach (string deviceId in devices)
                {
                    ...
                    List<string> instances = RegistryHelper.GetSubKeyNames<List<string>>(Registry.LocalMachine, devicePath);
                    if (instances == null)
                    {
                        continue;
                    }
                    ...
                    {
                        string driverRef = RegistryHelper.GetValue($@"HKEY_LOCAL_MACHINE\{devicePath}\{instanceId}", "Driver", string.Empty);
                        ...
                        string driverDesc = RegistryHelper.GetValue($@"HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\Class\{driverRef}", "DriverDesc", string.Empty);
                ...
                foreach ((string basePath, bool isCapture) in new[] { (@"SOFTWARE\Microsoft\Windows\CurrentVersion\MMDevices\Audio\Render", false), (@"SOFTWARE\Microsoft\Windows\CurrentVersion\MMDevices\Audio\Capture", true) })
                {
                    ...
                    if (regKey != null)
                    {
                        foreach (string subKeyName in regKey.GetSubKeyNames())
                        {
                            ...
                            if (subKey != null)
                            {
                                string value24 = RegistryHelper.GetValue(propsPath, "{a45c254e-df1c-4efd-8020-67d146a850e0},24", string.Empty);
                                string value8 = RegistryHelper.GetValue(propsPath, "{a8b865dd-2e3d-4094-ad97-e593a70c75d6},8", string.Empty);
                                string value5 = RegistryHelper.GetValue(propsPath, "{a8b865dd-2e3d-4094-ad97-e593a70c75d6},5", string.Empty);
                                string value6 = RegistryHelper.GetValue(propsPath, "{a8b865dd-2e3d-4094-ad97-e593a70c75d6},6", string.Empty);
                                string valueID = RegistryHelper.GetValue(propsPath, "{b3f8fa53-0004-438e-9003-51a46e139bfc},2", string.Empty);
                                ...
                                {
                                    string nameValue6 = RegistryHelper.GetValue(propsPath, "{b3f8fa53-0004-438e-9003-51a46e139bfc},6", string.Empty).Trim();
                                    string typeNameValue2 = RegistryHelper.GetValue(propsPath, "{a45c254e-df1c-4efd-8020-67d146a850e0},2", string.Empty).Trim();
