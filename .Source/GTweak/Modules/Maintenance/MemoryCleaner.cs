...
namespace GTweak.Modules.Maintenance
{
    ...
    internal sealed class MemoryCleaner
    {
        internal bool IsWinOldExists => Directory.Exists(PathTargets.Folders.WindowsOld);
        ...
        internal void StartMemoryCleanup(bool removeWindowsOld = false)
        {
            ...
            CommandExecutor.RunCommandAsTrustedInstaller(@$"/c rd /s /q {PathTargets.Folders.SystemDrive}Windows\Temp & " +
                @$"rd /s /q %localappdata%\Temp & rd /s /q {PathTargets.Folders.SystemDrive}Windows\ff*.tmp & rd /s /q {PathTargets.Folders.SystemDrive}Windows\History\* & " +
                $@"rd /s /q {PathTargets.Folders.SystemDrive}Windows\CbsTemp\* & rd /s /q {PathTargets.Folders.SystemDrive}Windows\System32\SleepStudy\* & " +
                $@"rd /s /q {PathTargets.Folders.SystemDrive}Users\%USERNAME%\AppData\Local\Microsoft\Windows\INetCache\IE\* & rd /s /q {PathTargets.Folders.SystemDrive}Windows\Downloaded Program Files\* & " +
                $@"del /f /q {PathTargets.Folders.SystemDrive}Windows\setupapi.log & rd /s /q {PathTargets.Folders.SystemDrive}Windows\Panther\* & " +
                $@"for /D %D in (""{PathTargets.Folders.SystemDrive}ProgramData\Microsoft\Windows\WER\*"") do (del /s /q ""%D\*.*"" & for /D %E in (""%D\*"") do rd /s /q ""%E"") & " +
                $@"del /f /q {PathTargets.Folders.SystemDrive}Windows\INF\setupapi.app.log & del /f /q {PathTargets.Folders.SystemDrive}Windows\INF\setupapi.dev.log & del /f /q {PathTargets.Folders.SystemDrive}Windows\INF\setupapi.offline.log");
