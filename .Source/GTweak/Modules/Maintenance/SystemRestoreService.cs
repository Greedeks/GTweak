...
namespace GTweak.Modules.Maintenance
{
    internal sealed class SystemRestoreService : TaskSchedulerManager
    {
        ...
        internal bool IsPointCreationAllowed => RegistryHelper.CheckValue(@"HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows NT\CurrentVersion\SystemRestore", "RPSessionInterval", "0");

        internal void CreateRestorePoint()
        {
            try
            {
                RegistryHelper.Write(Registry.LocalMachine, @"SOFTWARE\Microsoft\Windows NT\CurrentVersion\SystemRestore", "SystemRestorePointCreationFrequency", 0, RegistryValueKind.DWord);
        ...
        internal void DisableRestorePoint()
        {
            ...
            RegistryHelper.DeleteValue(Registry.LocalMachine, @"SOFTWARE\Microsoft\Windows NT\CurrentVersion\SPP\Clients", "{09F7EDC5-294E-4180-AF6A-FB0E6A0E9513}");
            ...
            DisableSR(PathTargets.Folders.SystemDrive + @"\\");
        }
        ...
        internal bool EnableRestorePoint(string errorKey = "error_recovery_noty")
        {
            try
            {
                RegistryHelper.DeleteFolderTree(Registry.LocalMachine, @"SOFTWARE\Policies\Microsoft\Windows NT\SystemRestore");
