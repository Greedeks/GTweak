    ...
    public partial class MoreView
    {
        ...
        private async void BtnLicenseWindows_ClickButton(object sender, EventArgs e)
        {
            if (WindowsLicense.IsWindowsActivated)
                new NotificationManager().Show("info", "ready_activate_notification").Perform();
            else
            {
                if (new SystemDiagnostics().IsNetworkAvailable())
                    await WindowsLicense.StartActivation();
                else
                    new NotificationManager().Show("warn", "network_activate_notification").Perform();
            }
        }
        ...
        private async void BtnRestorePoint_ClickButton(object sender, EventArgs e)
        {
            ...
            waitingWindow.Show();
            new NotificationManager().Show("info", "createpoint_notification").Perform();
        ...
        private async void BtnDisableRecovery_ClickButton(object sender, EventArgs e)
        {
            ...
            await backgroundQueue.QueueTask(delegate { new NotificationManager(300).Show("info", "disable_recovery_notification").Perform(); });
        }
            ...
            if (await NTFSCompressor.IsSupportNtfs(selectedPath))
            {
                if ((new DirectoryInfo(selectedPath).Attributes & FileAttributes.Compressed) != FileAttributes.Compressed)
                {
                    ...
                    await backgroundQueue.QueueTask(delegate
                    {
                        ...
                        catch { new NotificationManager().Show("warn", "error_compression_notification").Perform(); }
                    });
                    await backgroundQueue.QueueTask(delegate { new NotificationManager(500).Show("info", "success_compression_notification").Perform(); });
                }
                else
                    new NotificationManager().Show("info", "ready_compression_notification").Perform();
            }
            else
                new NotificationManager().Show("warn", "notsupport_ntfs_notification").Perform();
        }
        ...
        private async void BtnDecompression_ClickButton(object sender, EventArgs e)
        {
            ...
            if (await NTFSCompressor.IsSupportNtfs(selectedPath))
            {
                if ((new DirectoryInfo(selectedPath).Attributes & FileAttributes.Compressed) == FileAttributes.Compressed)
                {
                    ...
                    await backgroundQueue.QueueTask(delegate { new NotificationManager(500).Show("info", "success_decompression_notification").Perform(); });
                }
                else
                    new NotificationManager().Show("info", "ready_decompression_notification").Perform();
            }
            else
                new NotificationManager().Show("warn", "notsupport_ntfs_notification").Perform();
        }
